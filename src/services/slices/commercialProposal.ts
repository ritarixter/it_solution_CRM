...
export const commercialProposalSlice = createSlice({
  ...
  reducers: {
    addCommercialProposal(
      state,
      action: PayloadAction<ICommercialProposalState>
    ) {
    ...
    updateCommercialProposal(
      ...
      action: PayloadAction<{
        draggableElement: ICommercialProposalState | any;
        newIndex: number;
      }>
    ) {
      const oldIndex = state.draggableElements.indexOf(
        action.payload.draggableElement
      );
      state.draggableElements = update(state.draggableElements, {
        $splice: [
          [oldIndex, 1],
